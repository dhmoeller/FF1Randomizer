	...
	public class OverworldMap
	{
		...
		public void ShuffleEntrancesAndFloors(MT19337 rng, bool includeTowns, bool allowUnsafe = false)
		{
			...
			if (!includeTowns)
			{
				...
				placedMaps.Add(OverworldTeleportIndex.Lefein, TeleportShuffle.Lefein);
			}
			...
			var extraForks = destinations.Where(x => x.Teleports.Count() > 1).SelectMany(x => x.Teleports.Skip(1)).Count() + 1;
			...
			do
			{
				...
				shuffledExits = placedExits.ToDictionary(x => x.Key, x => x.Value);
				var teleports = new List<TeleportIndex>();
				if (!includeTowns) 
				{
					teleports.Add(TeleportIndex.SeaShrine1);
					while (teleports.Any())
					{
						...
						if (floor.Exit != ExitTeleportIndex.None)
							shuffledExits.Add(floor.Exit, TeleportShuffle.OverworldCoordinates[OverworldTeleportIndex.Onrac]);
					}
				}
				for (byte i = 0; i < mapCount; i++) {
					...
					teleports.AddRange(destination.Teleports.Where(x => !placedFloors.Keys.Contains(x)));
					while (teleports.Any())
					{
						var teleport = teleports.SpliceRandom(rng);
						var floor = destinations[mapCount + j];
